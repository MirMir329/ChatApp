...
const ChatList = () => {
    ...
    const { changeChat, resetShowInfo } = useChatStore()
    const { setIsMobileTrue } = useScreenWidthStore();
    ...
    const filteredChats = chats.filter((c) => {
        ...
        // return c.user.username.toLowerCase() === input.toLowerCase()
    })
            ...
            {filteredChats.map((chat) => (
                 <div className="item" key={chat.chatId} onClick={() => handleSelect(chat)} style={{backgroundColor: chat?.isSeen ? "transparent" : '#5183fe'}}>
                    ...
                    <div className="texts">
                        ...
                        <TextTrimmer text={chat.lastMessage} maxLength={20} />
                    </div>
                </div>
            ))}
           

            {addMode && <AddUser setAddMode={setAddMode} chats={chats}/>}
        </div>
    )
}
